...
## Arguments
- `$ARGUMENTS` - Example type: `python` (default), `rust`, `js`, or `all`
...
```bash
cd python-bindings && uv run python examples/basic_example.py
```

Or for the dict-based API example:
```bash
cd python-bindings && uv run python examples/example.py
```

### For `rust`:
Run the Rust example:
```bash
cargo run --example test_render -p dinja
```

### For `js`:
Run the JavaScript example:
```bash
cd examples/js-binding && node example.js
```
